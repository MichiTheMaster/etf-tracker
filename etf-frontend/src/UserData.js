  ...
  async function handleChangeEmail(e) {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/user/change-email`, {
        ...
        body: JSON.stringify({ currentPassword: emailPassword, newEmail: emailNew })
      });
      const data = await res.json();
      if (!res.ok) {
        setEmailError(data.error || 'Fehler beim Ändern der E-Mail');
      } else {
    ...
    <Stack spacing={2}>
      <Paper sx={{ p: 3 }}>
        ...
        <Stack spacing={1}>
          ...
          <Typography><strong>E-Mail verifiziert:</strong> {profile?.emailVerified ? 'Ja' : 'Nein'}</Typography>
          ...
          <Stack spacing={2}>
            ...
            <TextField
              ...
              InputProps={{
                endAdornment: (
                  <InputAdornment position='end'>
                    <IconButton size='small' onClick={() => setShowEmailPassword(v => !v)} edge='end'>
      ...
      <Paper sx={{ p: 3 }}>
        <Stack direction='row' spacing={2} alignItems='center' sx={{ mb: 2 }}>
          ...
          <Button size='small' variant='outlined' onClick={loadSessions}>Aktualisieren</Button>
